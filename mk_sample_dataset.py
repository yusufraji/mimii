...
Path.mkdir(Path.cwd() / config["sample_dataset_dir"] / 'min6db', parents=True, exist_ok=True)
Path.mkdir(Path.cwd() / config["sample_dataset_dir"] / '0db', parents=True, exist_ok=True)
Path.mkdir(Path.cwd() / config["sample_dataset_dir"] / '6db', parents=True, exist_ok=True)
...
for db in tqdm(db, desc='writing db files.', leave=False):
    for machine in tqdm(machine_type, desc='writing machine files.', leave=False):
        for id in tqdm(machine_id, desc='writing machine id files.', leave=False):
            ...
            sample_data['normal'].apply(create_dataset, dataset_dir=config["sample_dataset_dir"])
            # write dataset for the abnormal data
            sample_data['abnormal'].apply(create_dataset, dataset_dir=config["sample_dataset_dir"])
