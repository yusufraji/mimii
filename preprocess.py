...
for item in tqdm(wav_files, desc='preprocessing wav files.', leave=False):
    ...
    S_db = np.empty((config["feature"]["n_mels"], n_frames, config["feature"]["n_channels"]), dtype=np.float32)
    for j in range(config["feature"]["n_channels"]):
        S = librosa.feature.melspectrogram(y=wav[:,j], sr=config["feature"]["sr"], hop_length=config["feature"]["hop_length"])
